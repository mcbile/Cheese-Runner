...
// Green dollar sign texture for successful kill reward indicator
let cachedHitDollarTexture: THREE.CanvasTexture | null = null;
export const getHitDollarTexture = (): THREE.CanvasTexture => {
    if (cachedHitDollarTexture) return cachedHitDollarTexture;
    const canvas = document.createElement('canvas');
    canvas.width = 128;
    canvas.height = 128;
    const ctx = canvas.getContext('2d');
    if (ctx) {
        ...
        ctx.fillText('$', 64, 64);
        // Dark green outline for better visibility
        ctx.strokeStyle = '#009900';
        ctx.lineWidth = 2;
        ctx.strokeText('$', 64, 64);
    }
    cachedHitDollarTexture = new THREE.CanvasTexture(canvas);
    cachedHitDollarTexture.colorSpace = THREE.SRGBColorSpace;
    return cachedHitDollarTexture;
};

// Red dollar sign texture for projectile miss indicator
let cachedMissDollarTexture: THREE.CanvasTexture | null = null;
export const getMissDollarTexture = (): THREE.CanvasTexture => {
    if (cachedMissDollarTexture) return cachedMissDollarTexture;
    const canvas = document.createElement('canvas');
    canvas.width = 128;
    canvas.height = 128;
    const ctx = canvas.getContext('2d');
    ...
    cachedMissDollarTexture = new THREE.CanvasTexture(canvas);
    cachedMissDollarTexture.colorSpace = THREE.SRGBColorSpace;
    return cachedMissDollarTexture;
};
